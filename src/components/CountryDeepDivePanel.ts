...
export class CountryDeepDivePanel implements CountryBriefPanel {
  ...
  public updateHousingCycle(data: {
    ...
    if (data.residential) {
      grid.append(
        this.metricBox('Residential (real)', `${data.residential.indexValue.toFixed(1)}`),
        this.metricBox('Residential YoY', this.formatPctTrend(data.residential.yoyChange)),
      );
    }
    if (data.commercial) {
      grid.append(
        this.metricBox('Commercial (real)', `${data.commercial.indexValue.toFixed(1)}`),
        this.metricBox('Commercial YoY', this.formatPctTrend(data.commercial.yoyChange)),
      );
    }
    if (data.dsr) {
      grid.append(
        this.metricBox('Household DSR', `${data.dsr.dsrPct.toFixed(1)}%`),
        this.metricBox('DSR QoQ', this.formatPctTrend(data.dsr.change)),
      );
    }
  ...
  public updateNationalDebt(entry: { debtToGdp: number; debtUsd: number; annualGrowth: number; source: string } | null): void {
    ...
    const grid = this.el('div', 'cdp-pro-metric-grid');
    grid.append(
      this.metricBox('Debt-to-GDP', `${entry.debtToGdp.toFixed(1)}%`),
      this.metricBox('Total Debt', this.formatMoney(entry.debtUsd)),
      this.metricBox('YoY Growth', this.formatPctTrend(entry.annualGrowth)),
      this.metricBox('Source', entry.source),
    );
  ...
  public updateSanctionsPressure(data: { entryCount: number; sanctionsActive?: boolean } | null): void {
    ...
    const grid = this.el('div', 'cdp-pro-metric-grid');
    grid.append(
      this.metricBox('Sanctioned Entities', String(data.entryCount)),
      this.metricBox('Status', data.sanctionsActive ? 'Active' : 'None'),
    );
  ...
  public updateTariffTrends(data: { currentRate: number; trend: string; datapoints: Array<{ year: number; tariffRate: number }> } | null): void {
    ...
    const grid = this.el('div', 'cdp-pro-metric-grid');
    grid.append(
      this.metricBox('Effective Rate', `${data.currentRate.toFixed(2)}%`),
      this.metricBox('Trend', data.trend === 'rising' ? '\u2191 Rising' : '\u2193 Falling'),
    );
  ...
  private metricBox(label: string, value: string): HTMLElement {
