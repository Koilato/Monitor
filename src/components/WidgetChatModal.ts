...
import { getBrowserTesterKeys, getWidgetAgentKey } from '@/services/widget-store';
...
async function buildWidgetAuthHeaders(): Promise<Record<string, string>> {
  const testerKeys = getBrowserTesterKeys();
  const testerKey = testerKeys.length > 0 ? testerKeys[0] : undefined;
  const widgetKey = getWidgetAgentKey();
  if (testerKey || widgetKey) {
...
function renderExampleChips(container: HTMLElement, inputEl: HTMLTextAreaElement): void {
  container.innerHTML = '';
  const keys = EXAMPLE_PROMPT_KEYS;
...
function renderPreviewHtml(
  ...
  const rendered = wrapWidgetHtml(html, 'wm-widget-shell-preview');
